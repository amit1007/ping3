...
@Component({
  ...
  styleUrls: ['./alert-inbox.component.css']
})
...
ngOnInit() {
  ...
  // console.log("AuthService.loggedUserTableID>>>>", AuthService.loggedUserTableID);
      this.commanApi.getLoggeddUser().subscribe(res=>{
        ...
        this.api.getLoginIDWiseAlerts(res.PingUserID)
        .subscribe(res => {
          console.log(res);
  ...
  onTriggerChange(event,id){
    ...
    this.api.updatetriggerAlert(triggerStatus).subscribe(res=>{
        ...
        this._flashMessagesService.show(' successfully! Trigger ON',{cssClass:'alert-success',timeout:1000});
        }else{
        this._flashMessagesService.show('successfully! Trigger OFF',{cssClass:'alert-danger',timeout:1000});
        }
      }
    })
  }
